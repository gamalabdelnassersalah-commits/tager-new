...
const STORE_KEY = 'tager_platform_state';
const SESSION_KEY = 'tager_session';
const CART_KEY = 'tager_cart';
...
const DEFAULT_STATE = () => ({
  settings:{
    ...
    materials:['أرز','سكر','زيت','دقيق','مياه','عصائر','لبن','جبن','منظفات','ورقيات','معلبات','مواد تعبئة']
  },
...
function home(){
  ...
  <section class="page hero">
    ...
    <aside class="ops-card">
      ...
      <div class="steps">
        <div class="step"><b>1</b><div><strong>إنشاء الإدارة</strong><p>أول تشغيل من صفحة الإعداد، بدون أي حسابات جاهزة داخل المنصة.</p></div></div>
  ...
  <section class="page compact">
    ...
    <div class="grid-3">
      ...
      <div class="card lift"><div class="icon">🧭</div><h3>جاهزية الإطلاق</h3><p class="muted">قائمة تشغيل واضحة، إعدادات، سياسات، وحفظ ملف احتياطي من البيانات.</p></div>
    </div>
  </section>
  <section class="page compact">
...
function productsPage(){
  ...
  const render = () => {
    ...
    qs('#productsGrid').innerHTML = list.length ? list.map(p=>{
      ...
      return `<div class="card product-card">${productImage(p)}<div class="product-body"><h3>${esc(p.name_ar)}</h3><p class="muted">${esc(p.category||'بدون فئة')} — ${esc(p.material||'')} — ${esc(v.store_name||'مورد')}</p><div class="price">${money(productPrice(p,pt))}</div><p class="small"><b>المخزون:</b> ${esc(p.stock)} | <b>الوحدة:</b> ${esc(p.unit||'قطعة')}</p><div class="action-row"><button class="btn" onclick="addToCart('${esc(p.id)}','${esc(pt)}')">إضافة للسلة</button><button class="btn3" onclick="go('/product/${esc(p.id)}')">التفاصيل</button><button class="btn3" onclick="go('/vendor-page/${esc(p.vendor_id)}')">المورد</button></div></div></div>`;
...
function adminDashboard(){
  ...
  page('لوحة الإدارة', `<div class="grid-3">${metric('المستخدمون',state.users.length)}${metric('قيمة الطلبات',money(ordersTotal))}${metric('مستحقات المنصة',money(commission-paid))}${metric('موردون قيد المراجعة',state.users.filter(x=>x.role==='vendor'&&x.status==='pending').length)}${metric('منتجات قيد المراجعة',state.products.filter(x=>x.status==='pending').length)}${metric('تذاكر دعم مفتوحة',state.support_tickets.filter(t=>t.status==='open').length)}</div><div class="tabs"><button class="tab active" onclick="showPanel('overview')">الملخص</button><button class="tab" onclick="showPanel('users')">المستخدمون</button><button class="tab" onclick="showPanel('vendors')">الموردون</button><button class="tab" onclick="showPanel('products')">المنتجات</button><button class="tab" onclick="showPanel('materials')">الفئات والمواد</button><button class="tab" onclick="showPanel('orders')">الطلبات</button><button class="tab" onclick="showPanel('finance')">المالية</button><button class="tab" onclick="showPanel('reports')">التقارير</button><button class="tab" onclick="showPanel('settings')">الإعدادات</button><button class="tab" onclick="showPanel('support')">الدعم</button><button class="tab" onclick="showPanel('data')">البيانات</button><button class="tab" onclick="logout()">خروج</button></div><div id="panel-overview" class="panel">${adminOverview()}</div><div id="panel-users" class="panel hidden">${adminUsers()}</div><div id="panel-vendors" class="panel hidden">${adminVendors()}</div><div id="panel-products" class="panel hidden">${adminProducts()}</div><div id="panel-materials" class="panel hidden">${adminMaterials()}</div><div id="panel-orders" class="panel hidden">${adminOrders()}</div><div id="panel-finance" class="panel hidden">${adminFinance()}</div><div id="panel-reports" class="panel hidden">${adminReports()}</div><div id="panel-settings" class="panel hidden">${adminSettings()}</div><div id="panel-support" class="panel hidden">${adminSupport()}</div><div id="panel-data" class="panel hidden">${adminData()}</div><div id="out"></div>`);
  bindAdminForms();
}
function adminOverview(){
  return `<div class="grid"><div class="card"><h3>جاهزية التشغيل</h3><div class="steps"><div class="step"><b>✓</b><div><strong>بيانات حقيقية فقط</strong><p>النظام يبدأ من بياناتك الحقيقية، وكل شيء يتم إدخاله من خلال النماذج.</p></div></div><div class="step"><b>✓</b><div><strong>الإعدادات المالية داخل الإدارة</strong><p>نسب العمولة والسلة المميزة لا تظهر في الصفحات العامة.</p></div></div><div class="step"><b>✓</b><div><strong>مراجعة قبل النشر</strong><p>اعتماد المورد والمنتج قبل الظهور في السوق.</p></div></div></div></div><div class="card"><h3>تدفق العمل</h3><p class="muted">مورد يسجل ← الإدارة تعتمد المورد ← المورد يضيف مناطق ومنتجات ← الإدارة تعتمد المنتجات ← العميل يطلب ← النظام يتحقق من التغطية ← الطلب يدخل التشغيل والمالية.</p></div></div>`;
}
function adminUsers(){
...
function adminProducts(){
  const rows = state.products.map(p=>{ const v=getVendor(p.vendor_id)||{}; return `<tr><td>${esc(p.name_ar)}</td><td>${esc(v.store_name||'')}</td><td>${esc(p.category||'')}</td><td>${esc(p.material||'')}</td><td>${badge(p.status)}</td><td>${money(p.wholesale_price)}</td><td>${esc(p.stock)}</td><td class="inline-actions"><button class="btn-ok" onclick="setProductStatus('${esc(p.id)}','approved')">اعتماد</button><button class="btn-danger" onclick="setProductStatus('${esc(p.id)}','rejected')">رفض</button></td></tr>`; });
  return table(['المنتج','المورد','الفئة','المادة','الحالة','سعر الجملة','المخزون','إجراء'], rows);
}
...
function adminMaterials(){
  ...
  const mats = state.settings.materials || [];
  const catRows = cats.map(c=>`<tr><td>${esc(c)}</td><td>${state.products.filter(p=>p.category===c).length}</td></tr>`);
  const matRows = mats.map(m=>`<tr><td>${esc(m)}</td><td>${state.products.filter(p=>p.material===m).length}</td></tr>`);
  return `<div class="grid-2"><form class="card form" id="materialsForm"><h3>إدارة الفئات والمواد</h3><p class="muted">أضف الفئات والمواد التي يستخدمها المورد عند إضافة المنتجات. كل عنصر في سطر مستقل.</p>${textarea('categories','الفئات',cats.join('\n'),'')}${textarea('materials','المواد / الأنواع',mats.join('\n'),'')}<button class="btn">حفظ الفئات والمواد</button></form><div class="card"><h3>ضبط السوق</h3><p class="muted">هذه الصفحة مخصصة للإدارة فقط، وتتحكم في تنظيم المنتجات قبل ظهورها للعملاء.</p><div class="mini-list"><span>عدد الفئات <b>${cats.length}</b></span><span>عدد المواد <b>${mats.length}</b></span><span>منتجات تنتظر المراجعة <b>${state.products.filter(p=>p.status==='pending').length}</b></span></div></div></div><div class="grid-2"><div class="card"><h3>الفئات الحالية</h3>${table(['الفئة','عدد المنتجات'],catRows)}</div><div class="card"><h3>المواد الحالية</h3>${table(['المادة','عدد المنتجات'],matRows)}</div></div>`;
}
function adminReports(){
  const ordersByStatus = ['new','confirmed','preparing','shipped','delivered','cancelled'].map(s=>`<tr><td>${badge(s)}</td><td>${state.orders.filter(o=>o.status===s).length}</td><td>${money(state.orders.filter(o=>o.status===s).reduce((a,o)=>a+num(o.total),0))}</td></tr>`);
  const vendorRows = state.vendors.map(v=>{const b=vendorBalance(v.user_id); return `<tr><td>${esc(v.store_name)}</td><td>${state.products.filter(p=>p.vendor_id===v.user_id).length}</td><td>${money(b.sales)}</td><td>${money(b.remaining)}</td></tr>`;});
  return `<div class="grid-3">${metric('إجمالي الطلبات',state.orders.length)}${metric('منتجات منشورة',state.products.filter(p=>p.status==='approved').length)}${metric('موردون معتمدون',state.users.filter(u=>u.role==='vendor'&&u.status==='approved').length)}${metric('طلبات مسلمة',state.orders.filter(o=>o.status==='delivered').length)}${metric('مدفوعات معتمدة',money(state.commission_payments.filter(p=>p.status==='approved').reduce((s,p)=>s+num(p.amount),0)))}${metric('دعم مفتوح',state.support_tickets.filter(t=>t.status==='open').length)}</div><div class="grid-2"><div class="card"><h3>الطلبات حسب الحالة</h3>${table(['الحالة','العدد','القيمة'],ordersByStatus)}</div><div class="card"><h3>الموردون والمالية</h3>${table(['المورد','منتجات','مبيعات','متبقي للمنصة'],vendorRows)}</div></div>`;
}
...
function adminData(){
  return `<div class="grid"><div class="card"><h3>تصدير البيانات</h3><p class="muted">احتفظ بملف احتياطي من البيانات المدخلة من المتصفح.</p><div class="action-row"><button class="btn" onclick="exportJSON()">تصدير JSON</button><button class="btn2" onclick="exportCSV('products')">منتجات CSV</button><button class="btn2" onclick="exportCSV('orders')">طلبات CSV</button><button class="btn2" onclick="exportCSV('users')">مستخدمون CSV</button></div></div><div class="card"><h3>استيراد ملف JSON</h3><p class="muted">استخدم ملف تم تصديره سابقاً من نفس المنصة.</p><input type="file" id="importFile" accept="application/json"><button class="btn3" onclick="importJSON()">استيراد</button></div><div class="card"><h3>سجل العمليات</h3>${table(['العملية','الكيان','التاريخ'], state.audit_logs.slice(0,20).map(l=>`<tr><td>${esc(l.action)}</td><td>${esc(l.entity_type)}</td><td>${new Date(l.created_at).toLocaleString('ar-EG')}</td></tr>`))}</div></div>`;
}
function bindAdminForms(){
  const f=qs('#settingsForm');
  if(f){ f.addEventListener('submit', e=>{ e.preventDefault(); const data=Object.fromEntries(new FormData(f)); Object.assign(state.settings,{platformName:data.platformName,platformStatus:data.platformStatus,supportPhone:data.supportPhone,supportPhone2:data.supportPhone2,supportWhatsapp:data.supportWhatsapp,supportEmail:data.supportEmail,currency:data.currency,operationsSlaHours:num(data.operationsSlaHours),defaultCommissionPercent:num(data.defaultCommissionPercent),premiumCartPercent:num(data.premiumCartPercent),paymentMethods:data.paymentMethods.split(/\n|,/).map(x=>x.trim()).filter(Boolean)}); saveState(); log('update_settings','platform_settings','general'); qs('#out').innerHTML=notice('تم حفظ الإعدادات.','ok'); }); }
  const mat=qs('#materialsForm');
  if(mat){ mat.addEventListener('submit', e=>{ e.preventDefault(); const data=Object.fromEntries(new FormData(mat)); state.settings.categories=data.categories.split(/\n|,/).map(x=>x.trim()).filter(Boolean); state.settings.materials=data.materials.split(/\n|,/).map(x=>x.trim()).filter(Boolean); saveState(); log('update_materials','platform_settings','materials'); qs('#out').innerHTML=notice('تم حفظ الفئات والمواد.','ok'); adminDashboard(); }); }
...
function contactPage(){ page('تواصل معنا', `<div class="grid-2"><div class="card"><h3>بيانات التواصل</h3><p><b>الهاتف:</b> ${esc(state.settings.supportPhone)}</p><p><b>الهاتف الإضافي:</b> ${esc(state.settings.supportPhone2||'-')}</p><p><b>واتساب:</b> ${esc(state.settings.supportWhatsapp)}</p><p><b>البريد:</b> ${esc(state.settings.supportEmail)}</p></div><div class="card"><h3>خدمة العملاء والموردين</h3><p class="muted">للموردين: أرسل طلب الانضمام من صفحة تسجيل مورد. للعملاء: استخدم الدعم عند وجود أي استفسار عن الطلبات أو التوصيل.</p><div class="action-row"><a class="btn" href="/register/vendor">تسجيل مورد</a><a class="btn3" href="/support">فتح تذكرة دعم</a></div></div></div>`); }
...
function exportJSON(){ download(`tager-backup-${new Date().toISOString().slice(0,10)}.json`, JSON.stringify(state,null,2), 'application/json'); }
function importJSON(){
...
function exportCSV(type){
  let rows=[];
  if(type==='products') rows = state.products.map(p=>({id:p.id,name:p.name_ar,vendor:getVendor(p.vendor_id)?.store_name||'',status:p.status,category:p.category,material:p.material||'',retail_price:p.retail_price,wholesale_price:p.wholesale_price,stock:p.stock}));
