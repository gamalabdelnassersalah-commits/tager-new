...
for (const file of keep) fs.copyFileSync(path.join(root, file), path.join(out, file));

const assetDir = path.join(root, 'assets');
if (fs.existsSync(assetDir)) for (const asset of fs.readdirSync(assetDir)) fs.copyFileSync(path.join(assetDir, asset), path.join(out, 'assets', asset));
